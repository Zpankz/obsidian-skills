...
### 2. Avoid list/heading syntax in labels

Mermaid renderers (including Obsidian) commonly choke on markdown-like patterns inside labels.

- Avoid `"1. Something"` (number + dot + space). Prefer:
  - `"1.Something"` (no space), or
  - `"(1) Something"`, or
  - `"Step 1: Something"`
- Avoid `<br/>` in labels unless you’ve verified it renders in your target renderer.
- Avoid `## Heading`-like prefixes inside labels.

### 3. Subgraph naming + referencing

- Prefer `subgraph id["Display name"]` (simple `id`, friendly display label).
- When connecting, reference the **id**, not the display name.

### 4. Special characters

- If you need quotes/parentheses in labels, prefer simple text alternatives (e.g. avoid nested quotes).
- Keep labels short and plain; put detailed explanations in surrounding markdown.

### 5. Arrow types (semantics)

- `-->` main flow
- `-.->` optional/supporting flow
- `==>` emphasis (use sparingly)
- `~~~` invisible link (layout only)

### 6. Prefer Square Layout (정사각형 레이아웃)
...
- [ ] Subgraphs use `id["Display"]`, and edges reference the `id` (not the display name)
