...
description: Build real-time bidirectional streaming with the Gemini Live API. Covers WebSocket audio/video streaming, VAD, session management, and ephemeral tokens in Python and TypeScript.
---
...
## Related skills
- `gemini-api-dev` — for broader non-realtime Gemini API and SDK guidance
- `gemini-interactions-api` — for the newer non-Live Interactions API and background task flows
- `gemini-image-skill` — for Gemini-powered image generation in document or slide workflows
- `viva-examiner` — for an Obsidian plugin example built directly on Gemini Live voice interactions
- `viva-llm` — for a vault-integrated LLM plugin that mixes voice and provider orchestration concepts
