...
- IDs should be **unique** across all nodes + edges.
- Use **16-character lowercase hex** IDs (matching what Obsidian generates).
- Every edge’s `fromNode` / `toNode` must reference an existing node ID.
- Avoid overlaps (Obsidian Canvas becomes hard to use when nodes overlap).

### Overlap check (axis-aligned bounding boxes)
Two nodes overlap if both are true:
- X overlap: `x1 < x2 + w2` and `x1 + w1 > x2`
- Y overlap: `y1 < y2 + h2` and `y1 + h1 > y2`

If a layout is crowded, increase spacing (typical safe defaults: 320px+ horizontal, 200px+ vertical) or switch to a grid/timeline layout.
