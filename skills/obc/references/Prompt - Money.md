...
obsidian unresolved counts verbose format=json          # Referenced but uncreated — some may be product ideas
obsidian tags counts sort=count format=json | jq '.[0:20]'  # Where thinking is concentrated
```
...
```bash
obsidian search query="context" format=json     # Find any context or reference notes
obsidian search query="project" format=json     # Find project-related notes
obsidian search query="business" format=json    # Find business-related notes
obsidian search query="revenue" format=json     # Find revenue-related notes
```
...
- [[Prompt - Guests|/guests]] — Derives podcast guest suggestions from the questions alive in the vault